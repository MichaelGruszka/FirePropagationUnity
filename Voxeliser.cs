...
using UnityEditor;
#endif
using BoundHierarchy = GRUProject.Firepropagation.TriAABBOverlap.BoundHierarchy;
using Triangle = GRUProject.Firepropagation.TriAABBOverlap.Triangle;
...
namespace GRUProject.Firepropagation
{
    public class Voxeliser
    {
                            ...
                            if (cachedGridBounds.Intersects(objectLevelBounds[objectCnt].bound))
                            {
                                ...
                                for (int triCnt = 0; triCnt < triBounds.Length; triCnt++)
                                {
                                    ...
                                    if (TriAABBOverlap.Check(cachedGridBounds, triangle))
                                    {
                                        ...
                                        _voxelMap[x][y][z].IsAble = 1;
                                        _voxelMap[x][y][z].Index = new Vector3Int(x, y, z);
                                        Debug.Log("x y z saved " + _voxelMap[x][y][z].Index);
                                        _voxelMap[x][y][z].WorldPosition = cachedVec;
                                        didFind = true;
                                        break;
                                    }
                                }
        ...
        {
            _voxelMap = new FireVoxel[_xDensity][][];
            for (var x = 0; x < _xDensity; x++)
            {
                _voxelMap[x] = new FireVoxel[_yDensity][];
                for (var y = 0; y < _yDensity; y++)
                {
                    _voxelMap[x][y] = new FireVoxel[_zDensity];
                    for (var z = 0; z < _zDensity; z++)
                    {
                        _voxelMap[x][y][z] = new FireVoxel();
                    }
                }
            }
        }
